...
function filter(){
  let begin_price=Number($("#begin-price").val());
  let end_price = Number($("#end-price").val());
  console.log(begin_price, end_price);
  $("#all-content-product").html("");
  $("#all-content-product").append(
    `
  ...
  $.getJSON("/php/api/post/R_product.php", {}, function (data) {
    ...
    for (let i = 0; i < data.data.length; i++) {
      ...
      let price_p = Number(data.data[i].price);
      
      if(price_p > Number(end_price)){
        continue;
      }
      if(price_p < Number(begin_price)){
        continue;
      }
      console.log(price_p);
      if (cnt % 5 == 0) {
